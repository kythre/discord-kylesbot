module.exports = (bot) => {
  ...
  bot.commands.push(new bot.eris.Command("find", (msg, args) => {
    const member = bot.findMember(msg.channel.guild, args[0]);
    if (member) {
      let description = "";
      for (let i in member) {
        description += `<@${member[i].id}>`
      }
      bot.send(msg, {
        description
      });
    } else {
      bot.send(msg, "user not found");
    }
  }, {
    description: "does nothing"
  }));
