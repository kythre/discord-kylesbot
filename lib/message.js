module.exports = (bot) => {
  bot.embed = function (msg, primary, secondary) {
    ...
    bot._.defaultsDeep(embed, {
      // author: {name: text},
      footer: {text: msg.command ? msg.command.label : null},
    ...
    for (let i in embed.fields) {
      ...
      if (field.value.length > 1024) {
        embed.fields[i].value = field.value.substring(0, 1009) + "..." + field.value.substring(field.value.length - 3);
      }
    }
  ...
  bot.send = function (msg, text, embed) {
    return new Promise((resolve, reject) => {
      bot.createMessage(typeof msg === "string" ? msg : msg.channel.id, bot.embed(msg, text, embed)).then((nmsg) => {
